...
import { DependencyGraph, GraphNode, GraphEdge, CrawlResult } from '../core/types';
...
export class GraphEngine {
   ...
   */
  getDownstream(nodeId: string, maxDepth: number = 3): Array<{nodeId: string; distance: number; path: string[]}> {
    const visited = new Set<string>();
    const queue: Array<{id: string; distance: number; path: string[]}> = [{id: nodeId, distance: 0, path: [nodeId]}];
