...
export class EpochManager {
  ...
  private buildMerkleTree(artifacts: EpochArtifact[]): string[][] {
    ...
    let currentLevel = artifacts.map(artifact => 
      this.hashString(JSON.stringify({
        id: artifact.id,
        hash: artifact.hash,
   ...
   */
  verifyArtifactInEpoch(
    artifact: EpochArtifact,
    epoch: Epoch,
    proofPath: number[]
  ): boolean {
    const artifactHash = this.hashString(JSON.stringify({
      id: artifact.id,
      hash: artifact.hash,
      eventType: artifact.eventType,
      timestamp: artifact.timestamp,
  ...
  generateMerkleProof(artifact: EpochArtifact, epoch: Epoch): number[] | null {
    ...
    const leafIndex = epoch.artifacts.findIndex(a => 
      a.id === artifact.id && a.hash === artifact.hash
    );
   ...
   */
  calculateCostSavings(artifactCount: number, costPerTx: number = 0.0001): {
